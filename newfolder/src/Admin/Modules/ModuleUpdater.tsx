...
// Define prop types
interface ModuleUpdaterProps {
  currentName: string;
  handleUpdateModule: (newName: string) => void;
}

const ModuleUpdater: React.FC<ModuleUpdaterProps> = ({ currentName, handleUpdateModule }) => {
  const [showModal, setShowModal] = useState(false);
  const [newName, setNewName] = useState(currentName);

  const handleUpdate = () => {
    handleUpdateModule(newName); // Pass updated name
